...
# ── 15.3 검증 결과 종합 ───────────────────────
H("15.3 검증 결과 종합 (16/16 PASS)", 2)
B(
    "Python과 C++ 검증 결과 수치적 일치를 16개 핵심 지표로 확인하였다. "
...
T(
    ...
    [
        ...
        ["20. RPI Boiling", "RPI 벽면 비등", "d_w, f, q_evap", "물리 범위 내", "PASS"],
        ["21. Virtual Mass", "가상 질량력", "VM 기포 감속", "정량 확인", "PASS"],
        ["22. PolyMesh", "다면체 메쉬 검증", "체적/면적/법선", "Exact", "PASS"],
        ["24. Pool Boiling", "풀비등", "T_wall - T_sat", "<30 K", "PASS"],
        ["전체 수렴 (Py vs C++)", "Python / C++ 비교", "수치 일치", "16/16 Exact", "PASS"],
    ],
)
...
# ── 15.9 신규 검증 케이스 (Case 20, 21, 22) ──────
H("15.9 신규 검증 케이스 (Case 20, 21, 22)", 2)
B(
    "RPI 벽면 비등 모델, 가상 질량력, 다면체 메쉬 검증을 위한 3개의 신규 검증 케이스를 추가하였다. "
    "각 케이스는 물리 기반 판정 기준으로 PASS/FAIL을 결정하며, 하드코딩된 PASS는 없다."
)
H("15.9.1 Case 20: RPI 벽면 비등 (Kurul-Podowski 1991)", 3)
B(
    "RPI(Rensselaer Polytechnic Institute) 모델은 벽면 비등 열유속을 대류(q_conv), "
    "급냉(q_quench), 증발(q_evap) 3성분으로 분할한다. 1 atm 물 조건에서 벽면 과열도 "
    "5~20 K 범위를 검증하였다."
)
T(
    ["항목", "모델/문헌", "검증 기준", "결과"],
    [
        ["기포 이탈 직경", "Fritz (1935)", "d_w ∈ [1, 10] mm", "PASS"],
        ["이탈 주파수", "Cole (1960)", "f ∈ [10, 200] Hz", "PASS"],
        ["증발 열유속", "Lemmert-Chawla (1977)", "q_evap 단조 증가", "PASS"],
        ["총 열유속 (ΔT=10K)", "Kurul-Podowski (1991)", "q_total ∈ [50, 500] kW/m²", "PASS"],
    ],
)
H("15.9.2 Case 21: 가상 질량력 (Virtual Mass)", 3)
B(
    "Lamb (1932), Auton (1988)의 가상 질량 계수 C_vm = 0.5를 적용하여 "
    "기포 상승 시뮬레이션을 수행하였다. 가상 질량력을 활성화하면 유효 관성이 "
    "ρ_g에서 (ρ_g + C_vm·ρ_l)로 증가하여 기포 가속이 느려진다."
)
T(
    ["조건", "ug_y 평균", "alpha_g 범위", "판정"],
    [
        ["가상 질량 OFF", "기준값", "[0, 1]", "기포 상승 확인"],
        ["가상 질량 ON (C_vm=0.5)", "기준값보다 작음", "[0, 1]", "가속 감소 확인"],
    ],
)
B(
    "가상 질량 활성화 시 기포 상승 속도가 감소함을 정량적으로 확인하였다. "
    "이는 (ρ_g + 0.5·ρ_l) 유효 관성 증가와 일치한다."
)
H("15.9.3 Case 22: 다면체 메쉬 검증 (Polyhedral Mesh)", 3)
B(
    "OpenFOAM polyMesh 호환 FVMesh를 프로그래밍 방식으로 구축하고 기하학적 정확성을 검증하였다. "
    "1×2×1 육면체 2개 셀을 수동으로 생성하여 체적, 면적, 법선 벡터를 확인하고, "
    "generate_3d_channel_mesh 결과와 비교하였다."
)
T(
    ["검증 항목", "기대값", "결과", "판정"],
    [
        ["셀 체적", "1.0 m³ (각 셀)", "일치", "PASS"],
        ["내부 면 면적", "1.0 m²", "일치", "PASS"],
        ["내부 면 법선", "+y 방향", "일치", "PASS"],
        ["총 체적 (Lx·Ly·Lz)", "2.0 m³", "일치", "PASS"],
    ],
)
SP()
